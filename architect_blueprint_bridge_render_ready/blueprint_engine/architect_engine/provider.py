...
def _post_json(url, payload, user_id, api_key):
    ...
    try:
        return _request(url, payload, user_id, api_key, use_basic_auth=use_basic_auth)
    except HTTPError as exc:
        if use_basic_auth and exc.code in {401, 403, 405}:
            try:
                return _request(url, payload, user_id, api_key, use_basic_auth=False)
...
def fetch_full_bundle(intake: dict, config: dict) -> dict:
    ...
    wheel=_post_json(base+"/natal_wheel_chart",payload,uid,key)
